...
num_bandits=int(np.random.randint(low=5, high=20,size=1))
...
def bandit_solving_algo(num_bandits):
    ...
    '''
    iterations=0
    individual_iterations=np.zeros(shape=num_bandits, dtype=float)
    epsilon=0.1
    for m in range(10):
        for j in range(num_bandits):
            individual_iterations[j]+=1
            estimated_means[j]=((estimated_means[j]*(individual_iterations[j]-1))+bandit_simulator(j))/individual_iterations[j]
        iterations+=1

    while(iterations<=num_allowed_iterations):
        random_number=random.randint(1,1000)

        if(random_number in range(1,int(epsilon*1000))):
            j=random.randint(0,num_bandits-1)
            individual_iterations[j]+=1
            estimated_means[j]=((estimated_means[j]*(individual_iterations[j]-1))+bandit_simulator(j))/individual_iterations[j]
            epsilon=.1 - ((iterations-6)/(10*num_allowed_iterations))
        else:
            j=estimated_means.argmax()
            individual_iterations[j]+=1
            estimated_means[j]=((estimated_means[j]*(individual_iterations[j]-1))+bandit_simulator(j))/individual_iterations[j]
    ...
    for j in range(len(estimated_means)):
        estimated_means[j]=round(estimated_means[j],2)
    
    print("Individual Iterations are-\n",individual_iterations)
