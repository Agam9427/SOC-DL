...
def bandit_solving_algo(num_bandits):
    ...
    old_means=np.full(shape=num_bandits, fill_value=500)
    epsilon=0.01
    while(iterations<=num_allowed_iterations):
        for j in range(num_bandits):
            if(abs(old_means[j]-estimated_means[j])>epsilon or individual_iterations[j]<=100):
                iterations+=1
                individual_iterations[j]+=1
                old_means[j]=estimated_means[j]
